...
class CommentNewsClass extends ObjectModel
{
    ...
    public static $definition = [
        ...
        'fields' => [
            ...
            'news' => ['type' => self::TYPE_INT, 'validate' => 'isUnsignedId', 'required' => true],
            'actif' => ['type' => self::TYPE_INT, 'validate' => 'isInt'],
            'name' => ['type' => self::TYPE_STRING, 'validate' => 'isString', 'required' => true, 'size' => 255],
            'url' => ['type' => self::TYPE_STRING, 'validate' => 'isUrlOrEmpty', 'size' => 255],
            'comment' => ['type' => self::TYPE_HTML, 'validate' => 'isString'],
            'id_parent' => ['type' => self::TYPE_INT, 'validate' => 'isUnsignedId'],
            'is_admin' => ['type' => self::TYPE_BOOL, 'validate' => 'isBool'],
        ],
    ];
    ...
    public function registerTablesBdd()
    {
        ...
        if (!Db::getInstance()->Execute('
            ...
            VALUES
            (1, 1, NOW(), \'Lorem ipsum\', \'\', \'Aliquam eu porttitor justo.
    ...
    public static function getCountListeParentsOnly($only_actif = null, $only_news = null)
    {
        $context = Context::getContext();
        $multiboutique_filtre = 'AND n.`id_shop` = ' . (int) $context->shop->id;
    ...
    public static function getReplies($parent_id, $only_validated = true)
    {
        $context = Context::getContext();
        $multiboutique_filtre = 'AND n.`id_shop` = ' . (int) $context->shop->id;

        $validation_filter = $only_validated ? 'AND cn.`actif` = 1 ' : '';

        $liste = Db::getInstance(_PS_USE_SQL_SLAVE_)->ExecuteS('
            SELECT cn.*
            FROM `' . bqSQL(_DB_PREFIX_ . self::$table_static) . '` cn
            LEFT JOIN `' . bqSQL(_DB_PREFIX_) . 'prestablog_news` n ON (cn.`news` = n.`id_prestablog_news`)
            WHERE cn.`id_parent` = ' . (int) $parent_id . '
            ' . $validation_filter . '
            ' . $multiboutique_filtre . '
    ...
    public static function getListeNavigate($only_actif = null, $limit_start = 0, $limit_stop = null, $only_parents = false, $only_news = null)
    {
        $context = Context::getContext();
        $multiboutique_filtre = 'AND n.`id_shop` = ' . (int) $context->shop->id;
        ...
        $parent_filter = '';
        if ($only_parents) {
            $parent_filter = 'AND cn.`id_parent` = 0';
        }
        $news = '';
        if ($only_news) {
            $news = 'AND cn.`news` = ' . (int) $only_news;
    ...
    public static function insertComment($news, $date, $name, $url, $comment, $actif = -1, $id_parent = 0, $is_admin = 0)
    {
        return Db::getInstance()->execute('
            INSERT INTO `' . bqSQL(_DB_PREFIX_ . self::$definition['table']) . '`
                (
                    `news`,
                    `date`,
                    `name`,
