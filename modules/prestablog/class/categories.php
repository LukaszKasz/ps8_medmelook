...
class CategoriesClass extends ObjectModel
{
    ...
    public static $definition = [
        ...
        'fields' => [
            ...
            'parent' => ['type' => self::TYPE_INT, 'validate' => 'isUnsignedId'],
            ...
            'description' => ['type' => self::TYPE_HTML, 'lang' => true, 'validate' => 'isString'],
        ],
    ];
    ...
    public function registerTablesBdd()
    {
        ...
        if (count($langues) > 0) {
            ...
            $description = [
                1 => 'Sed eget pretium lectus, sed bibendum augue. In sollicitudin convallis blandit.
    ...
    public static function injectGroupsInCategorie($active_group, $categorie)
    {
        ...
        if (is_array($active_group) && count($active_group) > 0) {
    ...
    public static function removePopupLinkDeleted($categorie)
    {
        return Db::getInstance()->Execute('
                DELETE FROM `' . bqSQL(_DB_PREFIX_ . self::$table_static) . '_popuplink`
                WHERE `id_prestablog_categorie` = ' . (int) $categorie);
    ...
    public static function getAllCategoriesInBranche($liste, &$all_categories = [])
    {
        ...
        $branch_previous = null)
    {
        ...
        foreach ($liste as $value) {
            if ((int) $value[self::$identifier_static] == (int) Tools::getValue('idC')
                        || (
                            $branch_previous != null && strpos($value['branch'], chr((int) $branch_previous)) !== false
                            && strpos($value['branch'], chr(Tools::getValue('idC')) . '.') !== false
                        )
                    || strpos($value['branch'], chr(Tools::getValue('idC')) . '.') !== false) {
