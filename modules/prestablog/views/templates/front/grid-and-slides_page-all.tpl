...
<h1 class="prestablogcat">
    ...
    <span>{$NbNews|intval}
    {if $NbNews <> 1}
        {l s='articles' d='Modules.Prestablog.Grid'}
    {else}
        {l s='article' d='Modules.Prestablog.Grid'}
    {/if}
    {if isset($prestablog_categorie_obj)}
        {l s='in the category' d='Modules.Prestablog.Grid'}&nbsp;{$prestablog_categorie_obj->title|escape:'htmlall':'UTF-8'}
    {/if}
    </span>
  {/if}
</h1>
{if sizeof($news)}
    ...
    {else if $prestablog_config.prestablog_article_page==4}
        <ul id="blog_list_1-7" class="full_picture">
   {/if}
        {foreach from=$news item=news_item name=NewsName}
        <li class="blog-grid">
            <div class="block_cont">
                <div class="block_top">
                {if isset($news_item.image_presente) and $news_item.image_presente}
                    <a href="{PrestaBlogUrl id=$news_item.id_prestablog_news seo=$news_item.link_rewrite titre=$news_item.title}" title="{$news_item.title|escape:'htmlall':'UTF-8'}">
                        <picture>
                            ...
                            <img loading="lazy" src="{$prestablog_theme_upimg|escape:'html':'UTF-8'}thumb_{$news_item.id_prestablog_news|intval}.jpg?{$md5pic|escape:'htmlall':'UTF-8'}" alt="{$news_item.title|escape:'htmlall':'UTF-8'}">
                        </picture>
                    </a>
                {/if}
                </div>
                <div class="block_bas">
                    ...
                    <br />
                        <span class="date_blog-cat">{l s='Published :' d='Modules.Prestablog.Grid'}
                ...
                <div class="prestablog_more">
                    <a href="{PrestaBlogUrl id=$news_item.id_prestablog_news seo=$news_item.link_rewrite titre=$news_item.title}" class="blog_link"><i class="material-icons">search</i> {l s='Read more' d='Modules.Prestablog.Grid'}</a>
    ...
    {include file="$prestablog_pagination"}
{else}
    <p class="warning">{l s='Empty' d='Modules.Prestablog.Grid'}</p>
