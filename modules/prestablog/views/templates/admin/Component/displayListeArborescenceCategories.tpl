...
{foreach $listecat $value}
    ...
    {* {assign var="tosend" value="{$value['children']}" } *}

    {if count($value['children']) > 0}
        {* {assign var="file" value="{$smarty.current_dir}\displayListeArborescenceCategoriesSubBlocks.tpl" } *}
        
        {* version dev benoit *}
        {* {$prestablog->get_displayListeArborescenceCategoriesSubBlocks($value['children'], $decalage + 1, $listeidbranchdeploy)} *}
        {* version 4.4.6 *}
        {$prestablog->displayListeArborescenceCategories($value['children'], $decalage + 1, $listeidbranchdeploy)}

        {*{include file="{$file}" listecat="{$tosend}"}*}
    {/if}
