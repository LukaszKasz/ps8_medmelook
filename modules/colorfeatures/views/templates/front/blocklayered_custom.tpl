...
<div id="layered_block_left" class="block">
	<p class="title_block">{l s='Catalog' mod='colorfeatures'}</p>
			...
			<div>
				...
				<div id="enabled_filters">
					<span class="layered_subtitle" style="float: none;">
						{l s='Enabled filters:' mod='colorfeatures'}
					</span>
					<ul>
									...
									{if $filter.type == $filter_type && isset($filter.values)}
										{if isset($filter.slider)}
											{if $smarty.foreach.f_values.first}
												<li>
													<a href="#" data-rel="layered_{$filter.type}_slider" title="{l s='Cancel' mod='colorfeatures'}"></a>
													{if $filter.format == 1}
														{l s='%1$s: %2$s - %3$s'|sprintf:$filter.name:{displayPrice price=$filter.values[0]}:{displayPrice price=$filter.values[1]}|escape:'html':'UTF-8' mod='colorfeatures'}
													{else}
														{l s='%1$s: %2$s %4$s - %3$s %4$s'|sprintf:$filter.name:$filter.values[0]:$filter.values[1]:$filter.unit|escape:'html':'UTF-8' mod='colorfeatures'}
													{/if}
												</li>
											{/if}
										{else}
											{foreach from=$filter.values key=id_value item=value}
												{if $id_value == $filter_key && !is_numeric($filter_value) && ($filter.type eq 'id_attribute_group' || $filter.type eq 'id_feature') || $id_value == $filter_value && $filter.type neq 'id_attribute_group' && $filter.type neq 'id_feature'}
													<li>
														<a href="#" data-rel="layered_{$filter.type_lite}_{$id_value}" title="{l s='Cancel' mod='colorfeatures'}"><i class="icon-remove"></i></a>
														{l s='%1$s: %2$s' mod='colorfeatures' sprintf=[$filter.name, $value.name]}
													</li>
												{/if}
											{/foreach}
										{/if}
				...
				{foreach from=$filters item=filter}
					{if isset($filter.values)}
						...
						<ul id="ul_layered_{$filter.type}_{$filter.id_key}" class="col-lg-12 layered_filter_ul{if isset($filter.is_af_feature) && $filter.is_af_feature || isset($filter.is_color_group) && $filter.is_color_group} color-group{/if}">
							{if !isset($filter.slider)}
								{if $filter.filter_type == 0}
									{foreach from=$filter.values key=id_value item=value name=fe}
										{if $value.nbr || !$hide_0_values}
										<li class="nomargin {if $smarty.foreach.fe.index >= $filter.filter_show_limit}hiddable{/if} {if isset($filter.is_af_feature) && $filter.is_af_feature}col-lg-1{else}col-lg-12{/if}"
											{if isset($filter.is_af_feature) && $filter.is_af_feature} style="width: 8.33333%;min-width: inherit;" {/if}
										>
											{if isset($filter.is_af_feature) && $filter.is_af_feature || isset($filter.is_color_group) && $filter.is_color_group}
											...
											{/if}
											{if !isset($filter.is_af_feature) || !$filter.is_af_feature}
								...
								{else}
									...
									{else}
										<select class="select form-control" {if $filter.filter_show_limit > 1}multiple="multiple" size="{$filter.filter_show_limit}"{/if}>
											<option value="">{l s='No filters' mod='colorfeatures'}</option>
											{foreach from=$filter.values key=id_value item=value}
											{if $value.nbr || !$hide_0_values}
								...
								{else}
									...
									{foreach from=$filter.list_of_values  item=values}
										<li class="nomargin {if $filter.values[1] == $values[1] && $filter.values[0] == $values[0]}layered_list_selected{/if} layered_list" onclick="$('#layered_{$filter.type}_range_min').val({$values[0]});$('#layered_{$filter.type}_range_max').val({$values[1]});reloadContent();">
											- {l s='From' mod='colorfeatures'} {$values[0]} {$filter.unit} {l s='to' mod='colorfeatures'} {$values[1]} {$filter.unit}
										</li>
									{/foreach}
							...
							{if count($filter.values) > $filter.filter_show_limit && $filter.filter_show_limit > 0 && $filter.filter_type != 2}
								<span class="hide-action more">{l s='Show more' mod='colorfeatures'}</span>
								<span class="hide-action less">{l s='Show less' mod='colorfeatures'}</span>
							{/if}
						</ul>
					</div>
					{/if}
	...
	<div id="layered_ajax_loader" style="display: none;">
		<p>
			...
			<br />{l s='Loading...' mod='colorfeatures'}
		</p>
	</div>
</div>
{else}
