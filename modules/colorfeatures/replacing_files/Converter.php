...
class Converter
{
    ...
    public function getFacetsFromFilterBlocks(array $filterBlocks)
    {
        ...
        /* START colorfeatures added lines */
        $advanced_features = Db::getInstance()->executeS('SELECT * FROM `' . _DB_PREFIX_ . 'color_features`');
        $features = array();
        $feature_as_textures = array();
        foreach ($advanced_features as $feature) {
            if ($feature['is_texture']) {
                $feature_as_textures[$feature['id_feature_value']] = $feature['id_color_feature'] . '.' . $feature['texture_extension'];
            }
            $features[$feature['id_feature']][$feature['id_feature_value']] = $feature['value'];
        }
        /* END colorfeatures added lines */
            ...
            switch ($filterBlock['type']) {
                ...
                case self::TYPE_FEATURE:
                    ...
                    foreach ($filterBlock['values'] as $id => $filterArray) {
                        ...
                        /* START colorfeatures added lines */
                        if ($filterBlock['type'] == 'id_feature' && isset($features[$filterBlock['id_key']]) && isset($features[$filterBlock['id_key']][$id])) {
                            if (isset($feature_as_textures[$id])) {
                                $img_path = _MODULE_DIR_ . 'colorfeatures/img/';
                                $filter->setProperty('texture', $img_path . $feature_as_textures[$id]);
                            } else {
                                $filter->setProperty('color', $features[$filterBlock['id_key']][$id]);
                            }
                        }
                        /* END colorfeatures added lines */
